...
#SBATCH --output=slurm_outputs/explain-%u-%x-%j.txt
...
version=explain-$dataset-$model-$(date +%s%3N_%d_%m_%Y_%H_%M)
...
python -u train.py --dataset_dir $SLURM_TMPDIR --output_dir $version_path --n_epochs $epochs --batch_size $batch_size --dataset $dataset --model $model
