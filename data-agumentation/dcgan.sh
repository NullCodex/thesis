...
#SBATCH --time=0-10:58      # time (DD-HH:MM)
#SBATCH --output=slurm_outputs/cdcgan-%u-%x-%j.txt
...
version=dataug-$rotate-$translate-$scale-$horizontal_flip-$dataset-$(date +%s%3N_%d_%m_%Y_%H_%M)
...
python -u dcgan.py --dataset_dir $SLURM_TMPDIR --output_dir $version_path --transforms $rotate $translate $scale $horizontal_flip --dataset $dataset
